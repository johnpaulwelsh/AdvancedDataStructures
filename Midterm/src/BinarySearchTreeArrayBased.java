...
public class BinarySearchTreeArrayBased<T extends Comparable<? super T>>
	...
	protected ArrayList<T> sortedArrList;
	...
	public BinarySearchTreeArrayBased(T[] items) {
		...
		this.items = items;
		// a new ArrayList to store the in-order-sorted elements is created
		this.sortedArrList = new ArrayList<T>();
		// arrange the elements in items according to the rules of a BST
		fillBSTAB(items);
		// sort the elements into sortedArrList according to an in-order
		// traversal
		inOrderTraversalSort(0);
	}
	...
	public T findKth(int k) {
		return sortedArrList.get(k);
	}
	
	/**
	 * Method to perform an in-order traversal on items recursively, copying
	 * each element to the ArrayList sortedArrList as we go, which will be the
	 * list we search through when we call findKth.
	 * @param currentRoot the element in items we are current treating as the
	 *        root, so we can find its children
	 */
	private void inOrderTraversalSort(int currentRoot) {
		// stops the recursion if we go out of bounds
		if (currentRoot > items.length - 1) {
			return;
		}
		...
		// do current root
		sortedArrList.add(items[currentRoot]);
